...
const Offerpage = () => {
    ...
    const handleSubmit=async()=>{
        try{
          ...
          const {data}=await axios.post("https://ecommerce-backend-api.adaptable.app/offer/create-offer",offerdata);
          if(data?.success){
             getalloffers();
             setPhoto();
    ...
    const handleDelete=async(Id)=>{
        try{
          const {data}=await axios.delete(`https://ecommerce-backend-api.adaptable.app/offer/delete-offer/${Id}`);
            ...
            {offers.map((c)=>(
                <>
                    <div className='card w-75 p-3 mt-2'>
                        <img
                            src={`https://ecommerce-backend-api.adaptable.app/offer/getofferphoto/${c._id}`}
