...
const Createcategory = () => { 
  ...
  const handleSubmit=async(e)=>{
    e.preventDefault();
    try{
      const {data}=await axios.post("https://ecommerce-backend-api.adaptable.app/category/create-category",{name});
      if(data?.success){
  ...
  const handleUpdate=async(e)=>{
    e.preventDefault();
    try{
      const {data}=await axios.put(`https://ecommerce-backend-api.adaptable.app/category/update-category/${selected._id}`,{
        name:updatename,
      })
      if(data?.success){
  ...
  const handleDelete=async(id)=>{
    try{
       const {data} =await axios.delete(`https://ecommerce-backend-api.adaptable.app/category/delete-category/${id}`);
       if(data?.success){
